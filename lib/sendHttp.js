...
module.exports = function (p_tcode, p_cond, p_limit, p_conn, p_func) {
  ...
  const qstream = con.queryStream("SELECT t.pkey,o_stime, if( ifnull(m.thost2,IFNULL(c.thost,''))>'',ifnull(m.thost2,c.thost) ,dstip) dstip, if(ifnull(m.tport2,IFNULL(c.tport,0))>0, ifnull(m.tport2,c.tport), dstport) dstport,uri,method,sdata, rlen " + 
...
function dataHandle(rdata, qstream, pfunc) {
  ...
  shead2.forEach(v => {
    ...
    if (! /^(GET|POST|DELETE|PUT|PATCH|Host|Content-Length)/i.test(kv[0])) {
      let val = kv.slice(1).join(':') ;
      if (/^Cookie/i.test(kv[0])) val = change_cookie(val) ;
      if ( kv[0].length > 0) {
        options.headers[kv[0]] = val.trim();
        new_shead += kv[0] + ': ' + val.trim()  + '\r\n' ;
      }
    }
  });
