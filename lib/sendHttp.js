...
module.exports = function (p_tcode, p_cond, p_limit, p_conn, p_func) {
  con = p_conn;

  let tcnt = 0;
  let cnt = 0;
  let condi = p_cond > ' ' ? "and (" + p_cond + ")" : "";
  let vlimit = p_limit > ' ' ? ' LIMIT ' + p_limit : "";
  const qstr = "SELECT COUNT(*) cnt FROM ttcppacket where tcode = ? " + condi;

  if (p_limit > ' ') {
    tcnt = p_limit.split(',')[1] * 1;
    console.log("%s Start 테스트id(%s)(%s)(%s) data건수 (%d)", PGNM, p_tcode, condi, vlimit, tcnt);
  } else
    con.query(qstr, [p_tcode],
      (err, row) => {
        if (!err) {
          tcnt = row[0].cnt;
          console.log(PGNM, row[0]);
          console.log("%s Start 테스트id(%s)(%s)(%s) data건수 (%d)", PGNM, p_tcode, condi, vlimit, tcnt);
        } else
  ...
  const qstream = con.queryStream("SELECT t.pkey,o_stime, if( ifnull(m.thost2,'')> '',m.thost2,dstip) dstip, if(ifnull(m.tport2,0)>0, m.tport2, dstport) dstport,uri,method,sdata, rlen " + 
      "FROM ttcppacket t left join thostmap m on (t.tcode = m.tcode and t.dstip = m.thost and t.dstport = m.tport) " +
      "where t.tcode = ? " + condi + " order by o_stime  " + vlimit, [p_tcode]);
...
function dataHandle(rdata, qstream, pfunc) {
  ...
  const req = http.request(options, function (res) {
    ...
    res.on('end', () => {
      ...
      con.query("UPDATE ttcppacket SET \
                     rdata = ?, stime = ?, rtime = ?,  elapsed = ?, rcode = ? ,rhead = ?, rlen = ? ,dstip = ?, dstport = ?,cdate = now() where pkey = ? "
        , [rDatas, stime.toSqlfmt(), rtime.toSqlfmt(), svctime, res.statusCode, resHs, rsz, rdata.dstip, rdata.dstport, rdata.pkey]
        , (err, result) => {
          if (err)
  ...
  req.on('error', function (e) {
    ...
    con.query("UPDATE ttcppacket SET \
                      stime = ?, rtime = ?,  elapsed = ?, rcode = ? , rhead = ? ,dstip = ?, dstport = ?, cdate = now() where pkey = ?"
      , [stime.toSqlfmt(), rtime.toSqlfmt(), svctime, 999, e.message , rdata.dstip, rdata.dstport, rdata.pkey]
      , (err, result) => {
        if (err)
